#!/usr/bin/env python3
"""Fast textured-GLB renderer for very large meshes: subsamples faces, vectorized-ish raster
with UV texture sampling + z-buffer, multi-view. python tools/render_glb_fast.py <glb> <png> [maxfaces]"""
...
P,UV,I,tex=load_glb(sys.argv[1]); TH,TW=tex.shape[:2]; R=480
maxf=int(sys.argv[3]) if len(sys.argv)>3 else 400000
if len(I)>maxf:
    rng=np.random.default_rng(0); I=I[rng.choice(len(I),maxf,replace=False)]
tiles=[]
    ...
    for fi in order:
        ...
        for Y in range(y0,y1+1):
            for X in range(x0,x1+1):
                ...
                sh=0.55+0.45*max(0,z/(abs(zs).max()+1e-9))
                img[Y,X]=(tex[ty,tx]*sh).astype(np.uint8)
    tiles.append(img)
grid=np.concatenate([np.concatenate(tiles[:2],1),np.concatenate(tiles[2:],1)],0)
Image.fromarray(grid).save(sys.argv[2]); print("saved",sys.argv[2],"verts",len(P),"faces shown",len(I),"tex",tex.shape)
